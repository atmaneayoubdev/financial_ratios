...
class CfoRatioWidget extends StatefulWidget {
  const CfoRatioWidget({Key? key, required this.name}) : super(key: key);
  final String name;

  @override
  State<CfoRatioWidget> createState() => _CfoRatioWidgetState();
}

class _CfoRatioWidgetState extends State<CfoRatioWidget> {
  final ProfileDateController controller = Get.put(ProfileDateController());

  double result = 0;
  TextEditingController cfo = TextEditingController();
  TextEditingController averageCurrentLiabilities = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const ImageWidget(name: "cfoRatios"),
        const SizedBox(
          height: 10,
        ),
        SizedBox(
                ...
                onChanged: (value) {
                  setState(() {
                    result = double.parse(cfo.text) /
                        double.parse(averageCurrentLiabilities.text);
                  });
                },
                keyboardType: TextInputType.number,
                controller: averageCurrentLiabilities,
                decoration: InputDecoration(
                  contentPadding: const EdgeInsets.all(5),
                  border: InputBorder.none,
                  hintText: "Average current liabilities",
                  hintStyle:
                      GoogleFonts.tajawal(color: kPrimaryColor, fontSize: 20),
                ),
              ),
