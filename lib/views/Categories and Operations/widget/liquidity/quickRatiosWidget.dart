...
class QuickRatioWidget extends StatefulWidget {
  const QuickRatioWidget({Key? key, required this.name}) : super(key: key);
  final String name;

  @override
  State<QuickRatioWidget> createState() => _QuickRatioWidgetState();
}

class _QuickRatioWidgetState extends State<QuickRatioWidget> {
  final ProfileDateController controller = Get.put(ProfileDateController());

  bool iscalculated = false;
  double result = 0;
  TextEditingController cach = TextEditingController();
  TextEditingController markableSecrities = TextEditingController();
  TextEditingController netRecievable = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const ImageWidget(name: "quickRatios"),
        const SizedBox(
          height: 10,
        ),
        SizedBox(
        ...
        SizedBox(
            ...
            child: Container(
              ...
              child: TextField(
                keyboardType: TextInputType.number,
                controller: markableSecrities,
                decoration: InputDecoration(
                  contentPadding: const EdgeInsets.all(5),
                  border: InputBorder.none,
                  hintText: "Markable securities",
                  hintStyle:
                      GoogleFonts.tajawal(color: kPrimaryColor, fontSize: 20),
                ),
              ),
        ...
        SizedBox(
            ...
            child: Container(
              ...
              child: TextField(
                keyboardType: TextInputType.number,
                controller: netRecievable,
                decoration: InputDecoration(
                  contentPadding: const EdgeInsets.all(5),
                  border: InputBorder.none,
                  hintText: "Net Recievible",
                  hintStyle:
                      GoogleFonts.tajawal(color: kPrimaryColor, fontSize: 20),
                ),
              ),
        ...
        SizedBox(
            ...
            child: Container(
              ...
              child: TextField(
                onChanged: (value) {
                  setState(() {
                    result = (double.parse(cach.text) +
                            double.parse(markableSecrities.text) +
                            double.parse(netRecievable.text)) /
